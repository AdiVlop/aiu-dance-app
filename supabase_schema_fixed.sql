-- =====================================================
-- AIU DANCE - SCHEMA COMPLET CORECTATĂ (FĂRĂ ERORI)
-- =====================================================
-- Această schemă rezolvă definitiv problema cu instructor_id
-- Rulează-o completă în Supabase SQL Editor
-- =====================================================

-- 1. CURĂȚARE COMPLETĂ (OPȚIONAL - decomentează dacă ai probleme)
...
-- 3. CREAREA TABELELOR ÎN ORDINEA CORECTĂ

-- Tabela PROFILES (trebuie creată primul)
...
-- Verificare că tabela profiles a fost creată
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'profiles') THEN
        RAISE EXCEPTION 'Tabela profiles nu a fost creată!';
    END IF;
END $$;

-- Tabela COURSES (cu instructor_id corect)
...
-- Verificare că tabela courses a fost creată cu instructor_id
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'courses' AND column_name = 'instructor_id') THEN
        RAISE EXCEPTION 'Coloana instructor_id nu există în tabela courses!';
    END IF;
END $$;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'courses') THEN
        RAISE EXCEPTION 'Tabela courses nu a fost creată!';
    END IF;
    ...
    RAISE NOTICE 'Schema AIU Dance a fost creată cu succes!';
END $$;

-- =====================================================
-- SCHEMA COMPLET CORECTATĂ - FĂRĂ ERORI!
-- =====================================================
-- Această schemă rezolvă definitiv problema cu instructor_id
-- Rulează-o completă în Supabase SQL Editor
-- =====================================================
