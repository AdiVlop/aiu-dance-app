-- ===============================================
-- FINAL PAYMENT SYSTEM SCHEMA - ALL FIXES
-- Sistem complet de plăți cursuri cu toate reparațiile
-- ===============================================
...
-- 2. Tabelă pentru notificări (cu coloana 'read' corectă)
...
-- 4. Creează tabela enrollments dacă nu există
...
-- 5. Adaugă coloane lipsă la enrollments dacă nu există
DO $$ 
BEGIN
...
-- 6. Creează tabela qr_scans dacă nu există
...
-- 7. Adaugă coloane lipsă la qr_codes dacă nu există
DO $$ 
BEGIN
...
-- 8. Creează tabela attendance dacă nu există
...
-- 10. RLS Policies pentru course_payments
...
-- 11. RLS Policies pentru notifications
...
-- 12. RLS Policies pentru enrollments
ALTER TABLE public.enrollments ENABLE ROW LEVEL SECURITY;

-- Users pot vedea propriile înscrieri
DO $$ BEGIN
...
-- Users pot crea propriile înscrieri
DO $$ BEGIN
...
-- Admins pot vedea toate înscriierile
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'enrollments' AND policyname = 'Admins can view all enrollments') THEN
...
-- 13. Funcție pentru actualizarea updated_at
...
-- 14. Trigger pentru updated_at pe course_payments
...
-- 15. Funcție pentru crearea notificărilor automate
...
-- 16. Trigger pentru notificări la înscriere
...
-- 17. Funcție pentru notificări la autorizarea plăților în rate
...
-- 18. Trigger pentru notificări la actualizarea plăților
...
-- 21. Actualizează prețurile cursurilor
...
-- 22. QR codes demo data
INSERT INTO public.qr_codes (code, title, type, data, is_active, course_id) 
SELECT 
    'ATTENDANCE_' || EXTRACT(EPOCH FROM NOW())::text,
    'QR Attendance Demo',
    'attendance',
    jsonb_build_object('purpose', 'attendance', 'location', 'Sala 1'),
    true,
    c.id
FROM public.courses c
WHERE c.title ILIKE '%salsa%'
LIMIT 1
ON CONFLICT (code) DO NOTHING;

INSERT INTO public.qr_codes (code, title, type, data, is_active) VALUES 
('BAR_MENU_' || EXTRACT(EPOCH FROM NOW())::text, 'QR Bar Menu', 'bar_order', jsonb_build_object('purpose', 'bar_menu', 'table', '1'), true)
ON CONFLICT (code) DO NOTHING;

-- 23. QR scans demo data
INSERT INTO public.qr_scans (qr_code_id, user_id, scan_data)
SELECT 
    qr.id,
    p.id,
    jsonb_build_object('scan_time', NOW(), 'method', 'demo')
FROM public.qr_codes qr, public.profiles p
WHERE qr.type = 'attendance'
AND p.email = 'adrian@payai-x.com'
LIMIT 1
ON CONFLICT DO NOTHING;

-- 24. Verificare finală - afișează statistici
...
-- 25. Afișează cursurile cu prețuri
SELECT 
    title,
    category,
    price,
